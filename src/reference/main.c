...
/* set defaults for date and location */
      strcpy(date, "1997/06/28 00:00:00"); /*!!!starting time in DT + UT !!!*/
      strcpy(dend, "1997/07/05 00:00:00"); /*!!!end time in DT + UT !!!*/
      strcpy(date0, "1997/06/30 00:00:00"); /*!!!origin time in DT + UT !!!*/
      tstep=1.; /* time step in hours */
      UT = 0.;
      UT = +9.;
      ndays = 8;
      abmin0 = 2880.;  /* definition of time=0 in min */
      strcpy(outfile,"jun30-cel");
...
/*        u = 0.0005 -(1.2e-4/2.5)*(u+v);*/ /*--- do not forget to surpress */
/*        u = 1.5e-5 -(1.e-5/2.5)*(u+v);*/
/*        u = neck-bottom -(amp/2.5)*(u+v);      if( u<0 ) u = -u;   */
/*        u = 0.0002 -(1.e-4/2.5)*(u+v);      if( u<0 ) u = -u; */
/*        u = 1.5e-3 -(1.5e-4/2.5)*(u+v);      if( u<0 ) u = -u; */
/*        u = 5e-4 -(1.e-4/2.5)*(u+v);      */
/*        u = -2.e-3 -(3.e-3/2.5)*(u+v);      */
/*        u = 8.8e-3 -(2.0e-3/2.5)*(u+v);     */ /* fit to Sep. 28 */
        u = 0.13 -(0.008/2.5)*(u+v);      

        printf("%g %13e, %13e, %13e, %13e %13e\n", 
                abmin+abmin0,a,b,c,d,u);
        fprintf(outstr,"%g %13e, %13e, %13e\n", 
                abmin+abmin0,b,d,u);
